...
bool EW::isEQLConverged()
{
  ...
  if( myRank == 0 && m_iter_EQL > 0)
  {
    ...
    cout << indent << "------------- EQL Convergence Statistics ----------------"
         ...
         << indent << "                      max change in Vs across all nodes"
         << indent << endl;
   for (int ix = 0; ix < m_vsConv_EQL.size(); ix++)
   {
    ...
    cout << indent << "---------------------------------------------------------"
         << indent << "Convergence criteria: at least 99% of nodes" 
         << indent << "                      meeting convergence criteria"
         << indent << endl;
    for (int ix = 0; ix < m_vsConvTot_EQL.size(); ix++)
   {
    if (ix == 0)
      cout << indents <<                0.0 << " <= Vs <  " << m_vsBins_EQL[0] << " : " << m_vsConvTot_EQL[0] << " %" << endl;
    else if (ix == m_vsConv_EQL.size()-1)
      cout << indents << m_vsBins_EQL[ix-1] << " <= Vs                   : " << m_vsConvTot_EQL[ix] << " %" << endl;
    else
      cout << indents << m_vsBins_EQL[ix-1] << " <= Vs <  " << m_vsBins_EQL[ix] << " : " << m_vsConvTot_EQL[ix] << " %" << endl;
   }
  ...
  if (m_vsConvTot_EQL[0] > 99.0 &&
      m_vsConvTot_EQL[1] > 99.0 &&
      m_vsConvTot_EQL[2] > 99.0 &&
      m_vsConvTot_EQL[3] > 99.0)
      m_conv_EQL = true;
...
void EW::calculateEQLUpdate(vector<Source*> & a_Sources)
{
  ...
  vector<float_sw4> maxvsConv, avgvsConv, numvsConv;
  maxvsConv.resize(4);
  numvsConv.resize(4);
  for (int ix = 0; ix < maxvsConv.size(); ix++)
  {
    maxvsConv[ix] = 0.0;
    numvsConv[ix] = 0.0;
  }
  ...
  for( int g = 0 ; g < mNumberOfGrids; g++)
  {
    ...
    for( int k = m_kStart[g] ; k <= m_kEnd[g]; k++ )
    {
      for( int j = m_jStart[g] ; j <= m_jEnd[g]; j++ )
      {
        ...
        {
          // Get previous EQL mu value
          float_sw4 vs_old = sqrt(mMuOrig_EQL[g][1](i,j,k)/mRho[g](i, j, k));

          // Reset the material arrays
          mQs[g](i,j,k) = mQsOrig_EQL[g](i,j,k);
          mQp[g](i,j,k) = 2*mQsOrig_EQL[g](i,j,k);
          mMu[g](i,j,k) = mMuOrig_EQL[g][0](i,j,k);
             ...
             ) // If is a point in the eql domain
          {
            ...
            // ggmax value, and the original mu (in mMuOrig_EQL[0])
            mMu[g](i, j, k) = mMuOrig_EQL[g][0](i,j,k) * ggmax;
            mMuOrig_EQL[g][1](i,j,k) = mMuOrig_EQL[g][0](i,j,k) * ggmax;
            // we also save it in mMuOrig_EQL[1] to have an un-viscoelastic-ed
            // version to compute convergence statistics with in next iter.

            // Calculate and save convergence statistics based on vs
            // Note: the error is calculated relative to the "original" vs that doesn't change,
            // which is the variable "vs", to make the ratio more comparable
            // between iterations (otherwise as vs_old decreases, it would get
            // harder and harder to converge)
            float_sw4 vs_new = sqrt(mMu[g](i, j, k)/mRho[g](i, j, k));
            if (vs < m_vsBins_EQL[0])
            {
              maxvsConv[0] = max(maxvsConv[0], abs(vs_new-vs_old)/vs*100);
              if (abs(vs_new-vs_old)/vs*100 < m_convPercent_EQL)
                numvsConv[0]+=1.0;
            }
            else if (vs < m_vsBins_EQL[1])
            {
              maxvsConv[1] = max(maxvsConv[1], abs(vs_new-vs_old)/vs*100);
              if (abs(vs_new-vs_old)/vs*100 < m_convPercent_EQL)
                numvsConv[1]+=1.0;
            }
            else if (vs < m_vsBins_EQL[2])
            {
              maxvsConv[2] = max(maxvsConv[2], abs(vs_new-vs_old)/vs*100);
              if (abs(vs_new-vs_old)/vs*100 < m_convPercent_EQL)
                numvsConv[2]+=1.0;
            }
            else
            {
              maxvsConv[3] = max(maxvsConv[3], abs(vs_new-vs_old)/vs*100);
              if (abs(vs_new-vs_old)/vs*100 < m_convPercent_EQL)
                numvsConv[3]+=1.0;
            }
    ...
    MPI_Allreduce(&maxvsConv[0],&m_vsConv_EQL[0],1,m_mpifloat,MPI_MAX,m_cartesian_communicator);
    MPI_Allreduce(&maxvsConv[1],&m_vsConv_EQL[1],1,m_mpifloat,MPI_MAX,m_cartesian_communicator);
    MPI_Allreduce(&maxvsConv[2],&m_vsConv_EQL[2],1,m_mpifloat,MPI_MAX,m_cartesian_communicator);
    MPI_Allreduce(&maxvsConv[3],&m_vsConv_EQL[3],1,m_mpifloat,MPI_MAX,m_cartesian_communicator);

    MPI_Allreduce(&numvsConv[0],&m_vsConvTot_EQL[0],1,m_mpifloat,MPI_SUM,m_cartesian_communicator);
    MPI_Allreduce(&numvsConv[1],&m_vsConvTot_EQL[1],1,m_mpifloat,MPI_SUM,m_cartesian_communicator);
    MPI_Allreduce(&numvsConv[2],&m_vsConvTot_EQL[2],1,m_mpifloat,MPI_SUM,m_cartesian_communicator);
    MPI_Allreduce(&numvsConv[3],&m_vsConvTot_EQL[3],1,m_mpifloat,MPI_SUM,m_cartesian_communicator);
...
float_sw4 EW::calculateDarendeli(int i, int j, int k, int g, vector<Source*> & a_Sources)
{
  ...
  float_sw4 vs = sqrt(mMuOrig_EQL[g][1](i, j, k)/mRho[g](i, j, k));
  float_sw4 PL;
  if (vs <= 200)
    PL = 10.0;
  else if (vs <= 360)
  ...
  //   cout << "y  " << y << endl;
  //   cout << "z  " << z << endl;
  //   cout << "depth  " << ptDepth << endl;
  //   cout << "PL  " << PL << endl;
  //   cout << "rho  " << mRho[g](i, j, k) << endl;
  //   cout << "vs  " << vs << endl;
  //   cout << "gamma_r  " << gamma_r << endl;
  //   cout << "emax  " << mEmax[g](i, j, k) << endl;
  //   cout << "ggmax  " << ggmax << endl;
  //   cout << "new vs  " << sqrt(mMuOrig_EQL[g](i, j, k) * ggmax/mRho[g](i, j, k)) << endl;
  //   cout << " " << endl;
  // }
  ...
  float_sw4 qs = 1.0 / (2.0 * (xi/100.0));
  mQs[g](i, j, k) = max(qs, 10.0);
...
// 
//---------------------------------------------------------------
long long int EW::updateEmax(vector<Sarray*> &U)
{
  long long int updatedtmp = 0;
  for( int g = 0 ; g < mNumberOfGrids; g++)
  {
    #pragma omp parallel for reduction(+:updatedtmp)
    for( int k = m_kStartIntEQL[g] ; k <= m_kEndIntEQL[g]; k++ )
    {
      for( int j = m_jStartIntEQL[g] ; j <= m_jEndIntEQL[g]; j++ )
      {
        ...
        {
          float_sw4 x, y, z, vs, ptDepth;
          int supergrid_pts = static_cast<int>(m_supergrid_width/mGridSize[g]);
          
          vs = sqrt(mMuOrig_EQL[g][0](i, j, k)/mRho[g](i, j, k));
          getCoordinates(i, j, k, g, x, y, z);
          getDepth(x, y, z, ptDepth); // Gets total depth below free surface including topography
  ...
  long long int updated;
  MPI_Allreduce( &updatedtmp, &updated, 1, MPI_LONG_LONG_INT, MPI_SUM, m_1d_communicator );
  return updated;
} // end updateEmax
